...
| `%prep`        | 用于准备要构建的软件的命令或一系列命令，例如，在Source0中解压缩归档文件。该指令可以包含shell脚本。 |
