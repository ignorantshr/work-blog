...
*tags*：如果需要构建的包本身就是一个交叉编译器，需要使用--target参数指定架构
...
*tag*：只有构建系统没有时才执行`autoreconfig`，如果修改了`configure.ac`或者`Makefile.am`，执行make时会自动重新生成相关文件。`configure.ac`的语法参考Autoconf手册。
...
- 以`_PROGRAMS`结尾的特殊变量指定了Makefile应该构建的文件。Automake将这些变量称之为`primary`，其他的primary例如 _SCRIPTS， _DATA， _LIBRARIES 等等，对应不同的文件类型。
...
*tag*：文件中没有涉及到安装路径，请参考autocong手册
...
对于每个primary，都有一个带有`EXTRA_`前缀的primary，这个变量用于列出可能构建的对象列表，在configure中决定是否构建。
...
```make
	...
	xml_DATA = file.xml
```
`noinst_`前缀表示有问题的对象只构建不安装

`check_`表明有问题的对象不能被构建直到`make check`命令运行过。也不可安装
